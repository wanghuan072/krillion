...
export function SiteFooter() {
  return <footer className={styles.footer}>
    <div className={`${styles.container} ${styles.footerGrid}`}>
      ...
      <div><h2>Explore</h2>{primaryNavigation.map((item) => <Link key={item.path} href={item.path} prefetch={false}>{item.label}</Link>)}</div>
      <div><h2>Legal</h2>{legalNavigation.map((item) => <Link key={item.path} href={item.path} prefetch={false} rel="noopener noreferrer nofollow">{item.label}</Link>)}</div>
    </div>

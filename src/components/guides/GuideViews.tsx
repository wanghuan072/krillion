...
export function GuidesList({ guides, preview = false }: { guides: Guide[]; preview?: boolean }) {
  return <div className={styles.guideList}>{guides.map((guide) => <Link href={preview ? `/preview-internal/guide/${guide.slug}` : guidePath(guide.slug)} className={styles.guideCard} key={guide.id}>
